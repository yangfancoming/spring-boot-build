...
public class ApplicationContextAssert<C extends ApplicationContext> extends AbstractAssert<ApplicationContextAssert<C>, C> {
	...
	public ApplicationContextAssert<C> hasBean(String name) {
		...
		if (findBean(name) == null) {
			throwAssertionError(new BasicErrorMessageFactory("%nExpecting:%n <%s>%nto have bean named:%n <%s>%nbut found no such bean",getApplicationContext(), name));
		}
		return this;
	}
	...
	public ApplicationContextAssert<C> hasSingleBean(Class<?> type, Scope scope) {
		...
		if (names.length == 0) {
			throwAssertionError(new BasicErrorMessageFactory("%nExpecting:%n <%s>%nto have a single bean of type:%n <%s>%nbut found no beans of that type",getApplicationContext(), type));
		}
		if (names.length > 1) {
			throwAssertionError(new BasicErrorMessageFactory("%nExpecting:%n <%s>%nto have a single bean of type:%n <%s>%nbut found:%n <%s>",getApplicationContext(), type, names));
		}
		return this;
	}
	...
	public ApplicationContextAssert<C> doesNotHaveBean(Class<?> type, Scope scope) {
		...
		String[] names = scope.getBeanNamesForType(getApplicationContext(), type);
		if (names.length > 0) {
			throwAssertionError(new BasicErrorMessageFactory("%nExpecting:%n <%s>%nnot to have a beans of type:%n <%s>%nbut found:%n <%s>",getApplicationContext(), type, names));
		}
		return this;
	}
	...
	public ApplicationContextAssert<C> doesNotHaveBean(String name) {
		...
		try {
			...
			throwAssertionError(new BasicErrorMessageFactory("%nExpecting:%n <%s>%nnot to have a bean of name:%n <%s>%nbut found:%n <%s>",getApplicationContext(), name, bean));
	...
	public <T> AbstractObjectArrayAssert<?, String> getBeanNames(Class<T> type) {
		...
		return Assertions.assertThat(getApplicationContext().getBeanNamesForType(type)).as("Bean names of type <%s> from <%s>", type, getApplicationContext());
	}
	 ...
	 * @param <T> the bean type
	 * @param type the bean type
	 * @return bean assertions for the bean, or an assert on {@code null} if the no bean is found
	 * @throws AssertionError if the application context did not start
	 * @throws AssertionError if the application context contains multiple beans of the given type
	 */
	...
	public <T> AbstractObjectAssert<?, T> getBean(Class<T> type, Scope scope) {
		...
		String[] names = scope.getBeanNamesForType(getApplicationContext(), type);
		if (names.length > 1) {
			throwAssertionError(new BasicErrorMessageFactory("%nExpecting:%n <%s>%nsingle bean of type:%n <%s>%nbut found:%n <%s>",getApplicationContext(), type, names));
		}
	 ...
	 * @return bean assertions for the bean, or an assert on {@code null} if the no bean is found
	 * @throws AssertionError if the application context did not start
	 * @throws AssertionError if the application context contains a bean with the given name but a different type
	 */
	@SuppressWarnings("unchecked")
	public <T> AbstractObjectAssert<?, T> getBean(String name, Class<T> type) {
		if (this.startupFailure != null) {
			throwAssertionError(contextFailedToStartWhenExpecting("to contain a bean of name:%n <%s> (%s)", name, type));
		}
		Object bean = findBean(name);
		if (bean != null && type != null && !type.isInstance(bean)) {
			throwAssertionError(new BasicErrorMessageFactory("%nExpecting:%n <%s>%nto contain a bean of name:%n <%s> (%s)%nbut found:%n <%s> of type <%s>",getApplicationContext(), name, type, bean, bean.getClass()));
		}
		return Assertions.assertThat((T) bean).as("Bean of name <%s> and type <%s> from <%s>", name, type,getApplicationContext());
	}
	 ...
	 * @param <T> the bean type
	 * @param type the bean type
	 * @return bean assertions for the beans, or an assert on an empty {@code map} if the no beans are found
	 * @throws AssertionError if the application context did not start
	 */
	...
	public <T> MapAssert<String, T> getBeans(Class<T> type, Scope scope) {
		...
		return Assertions.assertThat(scope.getBeansOfType(getApplicationContext(), type)).as("Beans of type <%s> from <%s>", type, getApplicationContext());
	}
	...
	public ApplicationContextAssert<C> hasFailed() {
		if (this.startupFailure == null) {
			throwAssertionError(new BasicErrorMessageFactory("%nExpecting:%n <%s>%nto have failed%nbut context started successfully",getApplicationContext()));
		}
		return this;
	}
