...
/**
 * {@link InvocationHandler} used by {@link ApplicationContextAssertProvider} generated proxies.
 */
class AssertProviderApplicationContextInvocationHandler implements InvocationHandler {
	...
	public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
		...
		if (isGetSourceContext(method)) return getSourceContext(args);
		if (isGetStartupFailure(method)) return getStartupFailure();
		if (isAssertThat(method)) return getAssertThat(proxy);
		if (isCloseMethod(method)) return invokeClose();
